...
public class CancelRoadTripRequester implements Runnable {
    private String TAG = CancelRoadTripRequester.class.getName();
    private final CancelRoadTripRequest request;

    public CancelRoadTripRequester(CancelRoadTripRequest request) {
    ...
    public void run() {
        ...
        CZResponse<BaseResponse> czResponse = HTTPOperationController.cancelRoadTrip(request);
        if (czResponse != null) {
            BaseResponse response = (BaseResponse) czResponse.getResponse();
            if (response.getResponse_status() == AppConstant.STATUS_SUCCESS) {
                EventBus.getDefault().post(new EventObject(EventConstant.CANCELROADTRIP_SUCCESS, czResponse.getResponse()));
            } else if (response.getResponse_status() == AppConstant.STATUS_FAILURE) {
                EventBus.getDefault().post(new EventObject(EventConstant.CANCELROADTRIP_ERROR, czResponse.getResponse()));
            }
        } else {
