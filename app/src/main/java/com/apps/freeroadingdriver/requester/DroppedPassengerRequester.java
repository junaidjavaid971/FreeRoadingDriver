...
public class DroppedPassengerRequester implements Runnable {
    private String TAG = DroppedPassengerRequester.class.getName();
    private final DroppedPassengerRequest request;

    public DroppedPassengerRequester(DroppedPassengerRequest request) {
    ...
    public void run() {
        ...
        CZResponse<BaseResponse> czResponse = HTTPOperationController.droppedPassengerRequest(request);
        if (czResponse != null) {
            BaseResponse response = (BaseResponse) czResponse.getResponse();
            if (response.getResponse_status() == AppConstant.STATUS_SUCCESS) {
                EventBus.getDefault().post(new EventObject(EventConstant.DROPPED_PASSENGER_SUCCESS, czResponse.getResponse()));
            } else if (response.getResponse_status() == AppConstant.STATUS_FAILURE) {
                EventBus.getDefault().post(new EventObject(EventConstant.DROPPED_PASSENGER_ERROR, czResponse.getResponse()));
            }
        } else {
