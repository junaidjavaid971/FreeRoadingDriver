...
public class RejectRideRequester implements Runnable {
    private String TAG = RejectRideRequester.class.getName();
    private final RejectRideRequest request;

    public RejectRideRequester(RejectRideRequest request) {
    ...
    public void run() {
        ...
        CZResponse<BaseResponse> czResponse = HTTPOperationController.rejectRequest(request);
        if (czResponse != null) {
            BaseResponse response = (BaseResponse) czResponse.getResponse();
            if (response.getResponse_status() == AppConstant.STATUS_SUCCESS) {
                EventBus.getDefault().post(new EventObject(EventConstant.RIDE_REJECT_SUCCESS, czResponse.getResponse()));
            } else if (response.getResponse_status() == AppConstant.STATUS_FAILURE) {
                EventBus.getDefault().post(new EventObject(EventConstant.RIDE_REJECT_ERROR, czResponse.getResponse()));
            }
        } else {
